    ...
    pub fn random_balls(device: &wgpu::Device,config: &wgpu::SurfaceConfiguration)->Self{
        ...
        for a in -11..11{
            for b in -11..11{
                ...
                let center = Vec3::new(a as f32 + 0.9 * rng.random::<f32>(),0.2,b as f32 + 0.9 * rng.random::<f32>());
                if (center - Vec3::new(4.0,0.2,0.0)).length() > 0.9{
                    if mat < 0.8{
                        let albedo = Vec4::new(rng.random::<f32>(),rng.random::<f32>(),rng.random::<f32>(),1.0);
                        spheres.push(Sphere::new(
                            center,0.2,albedo,Vec4::ZERO,0.0,0.0
                        ));
                    }else if mat < 0.95{
                        let albedo = Vec4::new(rng.random_range(0.5..1.0),rng.random_range(0.5..1.0),rng.random_range(0.5..1.0),1.0);
